var app = new Vue({
    ...
    mounted: function () {
        ...
        $('.star-rating').rateYo({
            ...
            normalFill: "#262626",
            readOnly: readonly,
            "starSvg": '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" ' +
            'viewBox="0 0 24 24"><path d="M12 .288l2.833 8.718h9.167l-7.417 5.389 2.833 8.718-7.416-5.388-7.417 ' +
            '5.388 2.833-8.718-7.416-5.389h9.167z"/></svg>'
        })
    ...
    methods: {
        ...
        loadEpisode: function (episode_id, is_embed) {
            $.get('episode/'+episode_id+'/load?is_embed=' + is_embed,
                function (data) {
                    if (is_embed == 0) {
                        jwplayer().stop();
        ...
        setPlayerSource: function (movie_id, is_series, is_embed) {
            var that = this;
            $.get(base + '/video/'+movie_id+'/set_player_source?is_series=' + is_series + '&is_embed=' + is_embed,
                function (data) {
                    if (is_embed == 0) {
                        if (is_series == 0) {
